...
export function getAverageTemp(weatherStats, data){
    for (const year in data){
        ...
        weatherStats[year].averageTemp = Number((totalTemp/data[year].days.length).toPrecision(3))
    }
}
...
export function getAverageHumidity(weatherStats, data){
    ...
    weatherStats[year].averageHumidity = Number((totalHumidity/data[year].days.length).toPrecision(3))}
}
...
export function getHistoricalAverages(weatherStats){
    ...
    weatherStats['historical'].averageHumidity = Number((historicHumidity/i).toPrecision(3))
}
